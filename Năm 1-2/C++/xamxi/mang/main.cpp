...
////////////
void chen(int& n, int a[], int pos, int val)
{
    n++;
    for(int i = n; i > pos; i--)
        a[i] = a[i - 1];
...
void tong2lelt(int n, int a[])
{
    int i = 0;
    while(i < n - 1)
    {
        if (isOdd(a[i]) && isOdd(a[i+1]))
        {
            chen(n,a,i+1,a[i] + a[i+1]);
            i = i + 1;
        }
        i++;
    }
...
int main()
{
    ...
    cout << "Nhap a: ";
    cin >> a;
    cout << char(findMostFrequentChar(a,b));
}
