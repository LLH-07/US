...
public class AuthzForcePdpAdapter {
    ...
    private static final String UPDATE_ACTION_OBLIGATION_ID = "urn:ucon:obligation:update-action";
    private static final String ACTION_CODE_ASSIGNMENT_ID = "urn:ucon:obligation:action-code";
    ...
    public PolicyDecision evaluate(EvaluationContext ctx) {
        try {
            ...
            return new PolicyDecision(
                    ...
                    permit ? "PERMIT" : "POLICY_NOT_SATISFIED",
                    extractInstructions(result)
            );
        } catch (Exception e) {
            return new PolicyDecision(false, "INDETERMINATE", e.getMessage(), List.of());
        }
    }
    ...
    private List<EnforcementInstruction> extractInstructions(DecisionResult result) {
        List<EnforcementInstruction> instructions = new ArrayList<>();
        ...
        for (PepAction pepAction : result.getPepActions()) {
            ...
            if (!UPDATE_ACTION_OBLIGATION_ID.equals(pepAction.getId())) {
                continue;
            }

            String actionCode = findAssignmentValue(
                    pepAction,
                    ACTION_CODE_ASSIGNMENT_ID
            );

            if (actionCode == null || actionCode.isBlank()) {
                throw new IllegalStateException(
                        "Missing action code in obligation: " + UPDATE_ACTION_OBLIGATION_ID
                );
            }

            instructions.add(new EnforcementInstruction(
                    pepAction.getId(),
                    actionCode
            ));
        }
