...
function renderVolumeButtons(bookId){
  ...
  return bookVolumesOf(bookId)
    .map(v => {
      ...
      const readCount = Math.max(0, Number(v.read_count ?? 0) || 0);
      const canFull = canReadFullVolume(v);
      ...
      return `<article class="volume-card volume-tile ${canFull ? (sharedOnly ? 'is-shared' : 'is-owned') : 'is-preview'} access-${model.toLowerCase()}" role="button" tabindex="0" aria-label="${escapeHtml(label)} - ${escapeHtml(title)}" onclick="openSingleVolumePicker(${Number(bookId)}, ${Number(v.id)})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault(); openSingleVolumePicker(${Number(bookId)}, ${Number(v.id)});}">
        <div class="volume-tile-head">
          ...
          <span class="volume-tile-read-count">${readCount.toLocaleString('vi-VN')} lượt đọc</span>
        </div>
...
function fullAccessDenyMessage(errorInfo = {}){
  const reason = typeof errorInfo === 'string'
    ? errorInfo
    : (errorInfo?.reason || errorInfo?.message || '');

  switch(reason){
    case 'NO_AUTHORIZATION_GRANTED':
      return 'Bạn chưa được cấp quyền đọc full tập này theo chính sách hiện tại.';

    case 'POLICY_NOT_CONFIGURED':
      return 'Hệ thống chưa cấu hình chính sách phù hợp cho yêu cầu đọc này.';

    case 'POLICY_NOT_SATISFIED':
      return 'Yêu cầu đọc full hiện chưa thỏa chính sách truy cập.';

    case 'SESSION_NOT_ACTIVE':
    case 'DENY_OR_REVOKED':
    case 'SESSION_NOT_ACTIVE_OR_REVOKED':
    case 'DEPENDENCY_DELETED':
      return 'Phiên đọc đã hết hiệu lực hoặc đã bị thu hồi.';

    case 'RENTAL_FULL_REQUIRED':
      return 'Bạn cần gói thuê đang hoạt động để đọc full tập này.';

    case 'PAID_FULL_REQUIRED':
    case 'VOLUME_PURCHASE_REQUIRED':
      return 'Bạn cần mua tập này để đọc full.';

    case 'FULL_ACCESS_REQUIRED':
      return 'Bạn chưa có quyền đọc full tập này.';

    case 'OBLIGATION_FULFILLMENT_FAILED':
      return 'Hệ thống chưa ghi nhận được việc hoàn thành yêu cầu. Bạn vui lòng thử lại.';

    case 'UNSUPPORTED_OBLIGATION_TYPE':
      return errorInfo?.message || 'Giao diện hiện chưa hỗ trợ loại yêu cầu mà chính sách trả về.';

    case 'INVALID_REQUIRED_OBLIGATION':
      return errorInfo?.message || 'Thông tin obligation do hệ thống trả về chưa đầy đủ.';

    default:
      return 'Bạn chưa có quyền đọc full tập này theo chính sách hiện tại.';
  }
}

function fullAccessActionsHtml(){
  const book = bookById(readerState?.bookId);
  const volume = volumeById(readerState?.volumeId);
  const model = normalizeAccessModel(volume?.access_model_override || book?.default_access_model);
  const previewUrl = `#reader/${readerState?.volumeId}/preview`;
  const backButton = `<button class="reader-popup-back-link" onclick="closeReaderFriendlyPopupAndReturn()">Về thông tin sách</button>`;
  const previewButton = `<a class="reader-popup-button secondary" href="${previewUrl}" onclick="hideReaderFriendlyPopup()">Đọc preview</a>`;

  if(model === 'PAID'){
    return `<button class="reader-popup-button" onclick="purchasePaidVolume(${Number(readerState?.volumeId || 0)}, false); hideReaderFriendlyPopup();">Mua tập</button>${previewButton}${backButton}`;
  }

  if(model === 'RENTAL'){
    return `<a class="reader-popup-button" href="#subscription" onclick="hideReaderFriendlyPopup()">Quản lý gói thuê</a>${previewButton}${backButton}`;
  }

  return `${previewButton}${backButton}`;
}

function showFullAccessRequiredPopup(errorInfo = {}){
  const message = fullAccessDenyMessage(errorInfo);
  showReaderFriendlyPopup(message, fullAccessActionsHtml());

  if(errorInfo?.attempts?.length){
    console.table(errorInfo.attempts);
  }
}
...
async function startReadSession(
  ...
  const params = new URLSearchParams({
    ...
    volumeId: String(realVolumeId),
    mode: requestedMode.toUpperCase()
  });

  if(accessPath){
    params.set('accessPath', String(accessPath));
  }

  const res = await fetch(`${API_BASE}/read/try?${params.toString()}`, {
    method:'POST',
    headers
  });
  let data = await res.json().catch(() => ({}));
  ...
  if(!res.ok){
    const attempt = requestedMode === 'full' && typeof requiredObligationAttempt === 'function'
      ? requiredObligationAttempt(data)
      : null;

    if(attempt && obligationRetryCount < 5){
      const fulfillment = await fulfillRequiredObligations(attempt);

      if(!fulfillment.completed){
        const error = new Error('PRE_OBLIGATION_DECLINED');
        error.data = {
          reason: 'PRE_OBLIGATION_DECLINED',
          message: 'Bạn chưa hoàn thành yêu cầu bắt buộc trước khi truy cập.',
          userHandled: true
        };
        throw error;
      }

      // Retry only the candidate/access path whose obligations were completed.
      return startReadSession(
        uiVolumeId,
        requestedMode,
        fulfillment.accessPath || attempt.accessPath || null,
        obligationRetryCount + 1
      );
    }
...
async function loadReaderChunk(chunkNo){
  ...
  try{
    ...
    if(loadingEl) loadingEl.textContent = '';
    const errorInfo = error?.data || { reason: error?.message || '' };
    const reason = errorInfo.reason || error?.message || '';
    if(['SESSION_NOT_ACTIVE','DENY_OR_REVOKED','DEPENDENCY_DELETED'].includes(reason)){
      clearSavedReaderSessionId(readerState?.volumeId);
    }
    if(reason === 'PRE_OBLIGATION_DECLINED'){
      const fallbackHash = readerState?.returnHash || '#library';
      if(location.hash === fallbackHash) route();
      else location.hash = fallbackHash;
      return;
    }
    if(['FULL_ACCESS_REQUIRED','PAID_FULL_REQUIRED','RENTAL_FULL_REQUIRED','VOLUME_PURCHASE_REQUIRED','POLICY_NOT_SATISFIED','NO_AUTHORIZATION_GRANTED','DENY','SESSION_NOT_ACTIVE','DENY_OR_REVOKED','DEPENDENCY_DELETED','OBLIGATION_FULFILLMENT_FAILED','UNSUPPORTED_OBLIGATION_TYPE','INVALID_REQUIRED_OBLIGATION'].includes(reason) || readerState?.requestedMode === 'full'){
      showFullAccessRequiredPopup(errorInfo);
